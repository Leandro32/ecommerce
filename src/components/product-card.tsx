            ...
            <div className="flex items-center gap-1 mb-1">
              {[...Array(5)].map((_, i) => (
                <Icon 
                  ...
                  className={i < Math.floor(product.rating) ? "text-warning-500 text-xs" : "text-default-300 text-xs"}
                />
              ))}
              <span className="text-tiny text-default-500 ml-1">({product.reviewCount})</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              {product.originalPrice && product.originalPrice > product.price ? (
                <>
                  ...
                  <span className="text-default-400 text-tiny line-through">${product.originalPrice.toFixed(2)}</span>
                </>
              ) : (
                <span className="font-semibold">${product.price.toFixed(2)}</span>
              )}
