...
export const HeroPage: React.FC = () => {
  ...
  useEffect(() => {
    const desiredButtonCount =
      formData.buttonLayout === 'oneButton' ? 1 : formData.buttonLayout === 'twoButtons' ? 2 : 0;
      ...
      <CardBody>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <Input
            ...
            onValueChange={(value) => handleFormChange('heroImageUrl', value)}
          />
          <Select
            label="Button Layout"
            selectedKeys={[formData.buttonLayout]}
            onSelectionChange={(keys) => handleFormChange('buttonLayout', Array.from(keys)[0])}
          >
            <SelectItem key="none" value="none">None</SelectItem>
            <SelectItem key="oneButton" value="oneButton">One Button</SelectItem>
            <SelectItem key="twoButtons" value="twoButtons">Two Buttons</SelectItem>
          </Select>
          ...
          {formData.buttons.map((button, index) => (
            <div key={index} className="p-4 border rounded-md space-y-4">
              ...
              <Input
                ...
                onValueChange={(value) => handleButtonChange(index, 'buttonText', value)}
              />
              <Input
                label="Button Link"
                value={button.buttonLink}
                onValueChange={(value) => handleButtonChange(index, 'buttonLink', value)}
              />
              <div className="flex items-center space-x-2">
                <input 
                  ...
                  onChange={(e) => handleButtonChange(index, 'isExternal', e.target.checked)}
                  className="h-4 w-4"
                />
                <label htmlFor={`button-isExternal-${index}`}>External Link</label>
              </div>
              <Select
                label="Variant"
                selectedKeys={[button.variant]}
                onSelectionChange={(keys) => handleButtonChange(index, 'variant', Array.from(keys)[0])}
              >
