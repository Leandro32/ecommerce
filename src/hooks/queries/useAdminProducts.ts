...
const fetchAdminProducts = async (filters: ProductFilters): Promise<{ products: Product[], totalPages: number }> => {
  ...
  const endpoint = queryString ? `/api/v1/admin/products?${queryString}` : '/api/v1/admin/products';
...
export const useAdminProducts = (filters: ProductFilters) => {
  return useQuery<{ products: Product[], totalPages: number }, Error>({
    queryKey: ['adminProducts', filters], // Use a different query key
